...
class Node:
                    ...
                    with ThreadPoolExecutor(max_workers=10) as executor:
                        while self._is_running:
                            try:
                                ...
                                continue        # normal case: no incoming connection
                            except OSError:
        ...
        with conn:
            ...
            if data:
                ...
                if message['type'] == 'new_block':
                    ...
                    self.blockchain.add_transaction(**message['data'])
                    self.broadcast(message)
                    threading.Thread(target=self.poch_consensus, daemon=True).start()
                    conn.sendall(json.dumps({'status': 1}))
                elif message['type'] == 'get_chain':
                    chain_data = [block.to_dict() for block in self.blockchain.chain]
                    conn.sendall(json.dumps(chain_data).encode())
    ...
    def poch_consensus(self):
        """
        PoCH consensus with safety limits to prevent infinite loops.
        """
        tv = int(self.calculate_hash(int(self.blockchain.c_val, 16) + self.id), 16)
        
        max_iterations = 50  # Max attempts before giving up
        iteration = 0
        consensus_timeout = 30.0  # 30 second timeout
        ...
        while iteration < max_iterations:
            # Check timeout
            if time.time() - start_time > consensus_timeout:
                if DEBUG:
                    print(f"[{self.port}] TIMEOUT after {iteration} iterations")
                # Timeout - create block anyway and exit
                if self.id in self.blockchain.candidancies.values():
                    latest_block = self.blockchain.get_latest_block()
                    block = Block(len(self.blockchain.chain), latest_block.hash, int(time.time()), 
                                 self.blockchain.pending_transactions, "", random.randint(0, 1000), self.port)
                    self.blockchain.pending_transactions = []
            ...
            if not p_candidacies:
                # No candidacies yet, wait and retry
                if DEBUG and iteration % 10 == 0:
                    print(f"[{self.port}] Iter {iteration}: No candidacies")
                time.sleep(0.1)
                continue
            
            candidacies = [v for k, v in p_candidacies.items() 
                           if int(self.calculate_hash(int(self.blockchain.c_val, 16) + v), 16) 
                              <= self.blockchain.target]
            
            num_cand = len(candidacies)
            
            if DEBUG and iteration % 5 == 0:
                print(f"[{self.port}] Iter {iteration}: {num_cand} candidacies (target range: 6-15)")
            
            # Too many candidates - increase difficulty
            if num_cand > 15:
                self.blockchain.update_target(num_cand)
                time.sleep(0.1)  # Small delay before retry
                continue
            
            # Too few candidates - decrease difficulty
            if num_cand < 6:
                self.blockchain.update_target(max(1, num_cand))  # Prevent log10(0)
                time.sleep(0.1)
                continue
            
            # SUCCESS: 6 <= num_cand <= 15
            if 6 <= num_cand <= 15:
                result = 0
                d = 1
                ...
                while result == 0:
                    result = (self.blockchain.chain[-1].index + 
                             sum(cand**d for cand in sorted_candidates)) % (len(candidacies) + 1)
                    d += 1
                
                # Check if this node won
                if self.id == sorted_candidates[result - 1]:
                    if DEBUG:
                        print(f"[{self.port}] WON consensus after {iteration} iterations")
                    latest_block = self.blockchain.get_latest_block()
                    block = Block(len(self.blockchain.chain), latest_block.hash, int(time.time()), 
                                 self.blockchain.pending_transactions, "", random.randint(0, 1000), self.port)
                    self.blockchain.pending_transactions = []
