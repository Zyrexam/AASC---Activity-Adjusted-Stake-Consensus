# Activity Adjusted Stake Consensus Algorithm (Variant)
import random
import numpy as np
...
class Node:
    ...
    def mine(self, message=None):
        """
        Modified AASC (mAASC) mining function - VARIANT with explicit stake gossip.
        
        This is a variant of AASC that adds explicit stake synchronization through
        network messages. The main AASC algorithm (see aasc.py) derives activity
        metrics implicitly from the blockchain without explicit stake gossip.
        ...
        - Leader selection: O(v) (probabilistic selection based on cumulative scores)
        
        Note: This variant is NOT part of the main AASC proposal.
        """
        ...
        # Clamp value to [0, 1)
        value = min(value, 0.9999)
        value = max(value, 0)

        block_producer_index = np.searchsorted(CUMMULATIVE, value)
        # Ensure index is valid
        block_producer_index = min(block_producer_index, len(PORTS) - 1)
...
class NodeCLI(cmd.Cmd):
    ...
    # Note: PoEM-related commands removed - mAASC is pure on-chain, no ML model needed
    ...
    def do_addtx(self, arg):
        ...
        try:
            ...
            # Safety check: validators must exist
            if not self.node.blockchain.validators:
                print("No validators available yet. Wait for network to initialize.")
                return
